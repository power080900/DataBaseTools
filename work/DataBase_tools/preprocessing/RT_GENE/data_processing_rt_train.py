...
root = "Z:\\RT_GENE\\orgdata"
out_root = "Z:\\RT_GENE\\data\\RGB"
...
def ImageProcessing_RT():
    ...
    if not os.path.exists(os.path.join(out_root, "labeldata/train")):
        os.makedirs(os.path.join(out_root, "labeldata/train"))

    # Prepare outfiles' path of label
    label_outpath1 = os.path.join(out_root, "labeldata/train", "train1.label")
    label_outpath2 = os.path.join(out_root, "labeldata/train", "train2.label")
    label_outpath3 = os.path.join(out_root, "labeldata/train", "train3.label")
    label_outpath4 = os.path.join(out_root, "labeldata/train", "valid.label")
...
def ImageProcessing_Person(im_root, person_path, label_outpath):
    ...
    for num, label in enumerate(label_info):
        ...
        save_name_face = os.path.join(f"{person_path}/inpainted/face/{imgs_face[num]}")
        save_name_left = os.path.join(f"{person_path}/inpainted/left/{imgs_left[num]}")
        save_name_right = os.path.join(f"{person_path}/inpainted/right/{imgs_right[num]}")
