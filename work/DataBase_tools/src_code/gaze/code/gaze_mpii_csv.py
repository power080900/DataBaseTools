import os
import pandas as pd
import numpy as np

mpii = 'Z:\\MpiiGaze\\data\\grayscale\\labeldata'

df = pd.DataFrame(columns=['label','Gaze_x','Gaze_y'])
df1 = pd.DataFrame(columns=['label','Gaze_x','Gaze_y'])

mpii_labels = os.listdir(mpii)

for num, label in enumerate(mpii_labels):
    if num <= 5:
        print(num, '/', len(mpii_labels))
        label_path = os.path.join(mpii, label)
        mpii_df = pd.read_csv(label_path, sep=' ')
        # print(mpii_df.head())
        for i in range(len(mpii_df)):
            print(i, '/', len(mpii_df))
            file = mpii_df['Image'][i]
            gaze_x, gaze_y = mpii_df['Gaze_2D'][i].split(',')
            gaze_x = np.rad2deg(float(gaze_x))
            gaze_y = np.rad2deg(float(gaze_y))
            
            df1.loc[len(df1)] = [file, gaze_x, gaze_y]

        df = pd.concat([df,df1])
        df.to_csv('gaze_mpii2.csv', index=False)
        df1 = pd.DataFrame(columns=['label','Gaze_x','Gaze_y'])
    else:
        break
df.to_csv('gaze_mpii2.csv', index=False)