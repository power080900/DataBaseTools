...
root_dir = 'Z:\\DIS_300VW\\data\\RGB\\labeldata'
image = 'Z:\\DIS_300VW\\data\\RGB\\rawdata\\001\\001-000001.png'
...
for id_ in ids:
    ...
    df = pd.concat([df, df1])
    df.to_csv('iris_300vw.csv', index=False)
    df1 = pd.DataFrame(columns=['label','eye_side','iris_size','dpi'])

df1.to_csv('iris_300vw.csv', index=False)