...
root = "Z:\Gaze360_normalized\org_data\gaze360_dataset"
out_root = "Z:\Gaze360_normalized\data\RGB"
...
def ImageProcessing_Gaze360():
    ...
    for i in range(4):
        ...
        outfiles[i].write("Image Origin WhichEye 3DGaze 2DGaze\n")
    ...
    for i in range(total_num):
        ...
        face_path = os.path.join(out_root, "Image", category, "Face", f"{i+1}.jpg")
        left_path = os.path.join(out_root, "Image", category, "Left", f"{i+1}.jpg")
        right_path = os.path.join(out_root, "Image", category, "Right", f"{i+1}.jpg")
        
        cv2.imwrite(face_path, face)
        cv2.imwrite(left_path, lefteye)
        cv2.imwrite(right_path, righteye)
        ...
        save_str_left = " ".join([save_name_left, save_origin, "left", save_gaze, save_gaze2d])
        save_str_right = " ".join([save_name_right, save_origin, "right", save_gaze, save_gaze2d])

        outfiles[split_index[0, i]].write(save_str_left + "\n")
        outfiles[split_index[0, i]].write(save_str_right + "\n")
