...
root = "Z:\\mpiigaze\\org_data"
sample_root = "Z:\\mpiigaze\\org_data\\Evaluation Subset\\all list for eye image"
out_root = "Z:\\mpiigaze\\data"
scale =True
...
def ImageProcessing_MPII():
    ...
    for person in persons:
        ...
        im_outpath = os.path.join(out_root,"grayscale", "rawdata", person)
        label_outpath = os.path.join(out_root,"grayscale", "labeldata", f"{person}.label")
        if not os.path.exists(im_outpath):
            os.makedirs(im_outpath)
        if not os.path.exists(os.path.join(out_root, "grayscale", "labeldata")):
            os.makedirs(os.path.join(out_root, "grayscale", "labeldata"))
...
def ImageProcessing_Person(im_root, sample_list, im_outpath, label_outpath, person):
    ...
    outfile.write("Image Origin WhichEye Gaze_3D Gaze_2D Head_3D Head_2D Rmat Smat GazeOrigin\n")
    ...
    for count, info in enumerate(im_list):
        ...
        if which_eye == "left":
            save_name = os.path.join(im_outpath, str(im_number) + "_left.jpg")
        else:
            save_name = os.path.join(im_outpath, str(im_number) + "_right.jpg")

        
        cv2.imwrite(save_name, im_eye)
        ...
        save_str = " ".join([save_name, save_origin, save_flag, save_gaze, save_gaze2d,  save_head, save_head2d, save_rvec, save_svec, origin])
